...
#' Theme ggplot alternatif aux couleurs INRAE
#'
#' Ce second theme ggplot correspond aux couleurs issues de la charte graphique INRAE V3.
#'
#' @param base_size base size
#'
#' @return a ggplot theme
#' @import ggplot2
#' @export
theme_inrae2 <- function(base_size = 15) {
  ...
  white <- "#FFFFFF"
    grey  <- "#4e5352"
      inrae <- palette_inrae()["inrae_fonce"]
      inrae_clair <- palette_inrae()["inrae"]
      inrae_sombre <- palette_inrae()["inrae_sombre"]
      # Starts with theme_grey and then modify some parts
      theme_grey(base_size = base_size) %+replace%
        ...
        ggplot2::theme(
          ...
          # Base Inherited Elements
          line = ggplot2::element_line(colour = white, size = 0.5, linetype = 1,lineend = "butt"),
          rect = ggplot2::element_rect(fill = white, colour = blue,size = 0.5, linetype = 1),
          text = ggplot2::element_text(face = "plain",
          ...
          # Panel
          panel.background   = ggplot2::element_rect(fill = inrae_clair, color = NA),
          panel.border       = ggplot2::element_rect(fill = NA, size = rel(1/2), color = blue),
          panel.grid.major   = ggplot2::element_line(color = white, size = rel(1/5), linetype = "longdash"),
          panel.grid.minor   = ggplot2::element_blank(),
          panel.grid.minor.x = ggplot2::element_blank(),
          panel.spacing      = ggplot2::unit(.75, "cm"),
          ...
          legend.key         = ggplot2::element_rect(fill = white, color = NA),
          legend.position    = "top",
          legend.text = ggplot2::element_text(color = inrae_sombre),

          legend.title = ggplot2::element_text(face = "bold", color = inrae_sombre),
          ...
          # Plot
          plot.background = ggplot2::element_rect(fill = inrae_clair),
          plot.title = ggplot2::element_text(family = "Raleway",
                                             color = white,
                                             face = "bold",
                                             size = rel(1.2), hjust = 0,
